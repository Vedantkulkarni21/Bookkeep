...
# def list_users_for_admin(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
#     # ensure only admins can access
#     role = getattr(current_user, "role", None)
#     if role is None or str(role) != "admin":
#         raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")
#     users = crud.get_all_users(db)
#     return [{"id": u.id, "name": u.name, "email": u.email, "phone": u.phone, "role": str(u.role)} for u in users]    # ...existing code...
...
#         # Check if user role matches requested role
#         if str(user.role) != payload.role:
#             raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Not authorized as {payload.role}")
        
#         token = create_access_token(subject=user.email, role=str(user.role))
#         return {"access_token": token, "token_type": "bearer", "role": str(user.role), "user_id": user.id}
...
#         users = crud.get_all_users(db)
#         return [{"id": u.id, "name": u.name, "email": u.email, "phone": u.phone, "role": str(u.role)} for u in users]
